...
**Worker pool sizing (decided 10 Aug 2026):** driven by real alpha
client volume — one alpha client is working with 11 files per party,
not the 2-3 originally assumed. A fixed pool of **3 workers** (Web
Workers, per §4) is used rather than one worker per file — spawning 11
simultaneous workers for an 11-file session offers no speed benefit past
available CPU cores and adds contention. Pool size is a tunable
constant, not hardcoded, so it can be adjusted after alpha testing shows
real per-file hashing times.

**Dispatcher rule:** when a worker slot frees up, it pulls the
**oldest queued file** (lowest `fileSeq`) next — not necessarily the
file the user most recently finished mapping. This keeps queued files
strictly ordered by confirm time regardless of how fast the user maps
subsequent files, which is what keeps the ordered mapping-file-write
guarantee above holding without extra bookkeeping. No preemption — a
file that starts processing in a slot runs to completion.

**Per-file status now has four values, not three:** `queued |
processing | ready | acknowledged`. The build-plan's original three-value
list (`queued | processing | acknowledged`) collapsed "processing
finished" and "user has acknowledged" into one state, but these are
genuinely different — a file can sit in `ready` for a while if the user
is still mapping a later file, and the UI needs to distinguish "done
processing, waiting on you" from "fully wrapped up." The re-download/
acknowledge prompt for a `ready` file surfaces only at the "load another
file" / "Done" choice point above, never mid-mapping of a later file.
...
`fileSeq` segment above — the fix (fileSeq-at-queue-entry assignment,
§3a) is designed as of 10 Aug 2026 but not yet implemented in code; the
Phase 3 ES-module conversion pass explicitly left this untouched.
